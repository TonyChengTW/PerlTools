...
#   Writer  : Mico Cheng
#   Version : 2005051701
#   Use for : mail blocking notify to spammer.
#   Host : pop3 server
#  ----------------------------------
use DBI;
#----------------------------------------------------------------
until ($#ARGV == 1) {
     print "\nUsage:  .pl  <account list>  <notify_spammer file>\n";
print "account list:  file name with accounts\n";
print "notify_spammer files   :  notify_spammer pattern file\n";
exit 1;
}
...
while (<ACCESSLIST>) {
    chomp;
		...
		&post2mdir($s_mailid, $mhost, $notify_spammer_file);
}

sub post2mdir {
    ($s_mailid, $mhost, $notify_spammer_file) = @_;
    ...
    $mdir_file = sprintf("%s/%d.%05d1.00000000.00.00.%s",
		...
		system "cp $notify_spammer_file $mdir_file";
    system "/usr/local/bin/chown rmail:rmail $mdir_file";
    print "Info: $cnt ==> $s_mailid sent notify_spammer!\n";
		$cnt--;
}
