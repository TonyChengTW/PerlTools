...
# Writer:  Miko Cheng
# Version: 20041122
# use for: backup aptg Data Level I to Tape
...
use constant FTP_DIR_APTG => '/f:/level_a/miko/aptg_mail';
use constant FTP_DIR_EZML => '/f:/level_c/miko/ezmail';
...
### create ftp object
@backup_list = `/usr/local/bin/find /backup/etc -type f -mtime -1`;
...
#----------------------------------------------------------------------------
### create ftp object
chomp($backup_file = `ls -t /backup/ezml/|head -1`);
$backup_file = '/backup/ezml/'.$backup_file;

$ftp->cwd(FTP_DIR_EZML) or die "Err2: $date $ftp->message:$!\n";
print "Info: uploading $backup_file.....";
$ftp->put($backup_file) or die "Err4: $date $ftp->message:$!\n";
print "OK!\n";
